...
tools: ['execute/runTask', 'execute/runInTerminal', 'execute/runTests', 'agent']
---
...
**IMPORTANT**: Analyze the skills catalog at `./.agents/skills/*` and activate the skills that are needed for the task during the process.
...
Example:
```bash
node ./.agents/scripts/set-active-plan.cjs plans/260225-feature-name
```
